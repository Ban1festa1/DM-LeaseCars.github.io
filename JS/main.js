...
if (tabsContainer && tabsContent) {
    ...
    initCardEffects(allCards); // Инициализация 3D-эффектов
    ...
    cardsByCategory['All'] = [...allCards];
    
    allCards.forEach(card => {
        const category = card.dataset.category || 'Car and Minivan'; 
        if (!cardsByCategory[category]) cardsByCategory[category] = [];
        cardsByCategory[category].push(card);
        card.classList.add('hidden'); 
    });

    // Функция показа карточек с задержкой
